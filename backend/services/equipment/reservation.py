"""Service that manages reservations in the equipment space."""
...
class ReservationService:
    ...
    def get_reservation(self, subject: User, id: int) -> Reservation:
        ...
        for user in reservation.users:
            if user.id == subject.id or self._permission_svc.check(
                subject, "equipment.reservation.read", f"user/{user.id}"
            ):
                has_permission = True
                break

        if not has_permission:
            raise UserPermissionException("equipment.reservation.read", "user/")
    ...
    def get_current_reservations_for_user(
        ...
        the given user. The permission needed is action "equipment.reservation.read" and
        resource "equipment.reservation.users/:focus_id"
    ...
    def _get_active_reservations_for_user(
        ...
        reservations = (
            ...
            .options(
                joinedload(ReservationEntity.users),
    ...
    def get_equipment_reservations(
        self, equipment: Sequence[Equipment], time_range: TimeRange
    ) -> Sequence[Reservation]:
        """Returns all reservations for a set of equipment in a given time range.

        Args:
            equipment (Sequence[Equipment]): The list of equipment to query for reservations.
            time_range (TimeRange): The date range to check for matching reservations.

        Returns:
            Sequence[Reservation]: All reservations for the equipment within the given time_range, including overlaps.
        """
        reservations = (
            self._session.query(ReservationEntity)
            .join(ReservationEntity.equipment)
            .filter(
                ReservationEntity.start < time_range.end,
                ReservationEntity.end > time_range.start,
                ReservationEntity.state.not_in(
    ...
    def equipment_availability(
        self, equipment: Sequence[Equipment], bounds: TimeRange
    ) -> Sequence[EquipmentAvailability]:
        """Returns a list of all equipment availability for specific equipment within a given timerange.

        Args:
            bounds (TimeRange): The time range of interest.
            equipment (list[Equipment]): The equipment to check the availability of.

        Returns:
            Sequence[EquipmentAvailability]: All equipment availability ordered by nearest and longest available.
        """
        # No equipment are available in the past
        now = datetime.now()
        if bounds.end <= now:
            return []
        ...
        # Start from a position where all equipment begin with same availability as
        # open_availability_list. From there, reservations will subtract availability
        # from the given equipment.
        equipment_availability_dict = self._initialize_equipment_availability_dict(
            equipment, open_availability_list
        )

        # Get all active reservations during the availability bounds for the equipment.
        ...
        reservations = self.get_equipment_reservations(equipment, reservation_range)

        # Subtract all equipment reservations from their availability
        self._remove_reservations_from_availability(
            equipment_availability_dict, reservations
        )

        # Remove equipment with availability below threshold
        available_equipment: list[EquipmentAvailability] = list(
            self._prune_equipment_below_availability_threshold(
                list(equipment_availability_dict.values()),
                self._policy_svc.minimum_reservation_duration()
                - MINUMUM_RESERVATION_EPSILON,
            )
        )
        ...
        # The rationale for entropy is when XL is wide open for walkins, within the given equipment search
        # we'd like to mix up the order in which equipment are assigned rather than always giving away
        # the same sequence of equipment (and causing more consisten wear and tear to it).
    ...
    def draft_reservation(
        ...
        "equipment.reservation.manage" for resource "user/{user.id}".
        ...
        Future work:
            ...
            * Multi-user reservations
                * Check for equality between users and available equipment
                * Limit users and equipment counts to policy
        ...
        if subject.id not in [user.id for user in request.users]:
            for user in request.users:
                self._permission_svc.enforce(
                    subject, "equipment.reservation.manage", f"user/{user.id}"
                )
        ...
        # Here we constrain the reservation start/end to that of the best available equipment requested.
        # This matters as walk-in availability becomes scarce (may start in the near future even though request
        # start is for right now), alternatively may end early due to reserved equipment on backend.
        equipment_entities = [
            self._session.get(EquipmentEntity, equipment_availability[0].id)
        ]
        bounds = equipment_availability[0].availability[0]
    ...
    def change_reservation(
        ...
        Future work:
            Implement the ability to change equipment, party, and start/end time within policy restrictions
        """
        ...
        if subject.id not in user_ids:
            for user_id in user_ids:
                self._permission_svc.enforce(
                    subject, "equipment.reservation.manage", f"user/{user_id}"
                )
        ...
        # Handle Requested Equipment Changes?
        if delta.equipment is not None:
            raise NotImplementedError("Changing equipment not yet supported.")
        ...
        if (
            delta.start is not None
            or delta.end is not None
            or delta.equipment is not None
        ):
    ...
    def list_all_active_and_upcoming(self, subject: User) -> Sequence[Reservation]:
        ...
        """
        self._permission_svc.enforce(subject, "equipment.reservation.read", f"user/*")
        now = datetime.now()
        reservations = (
            self._session.query(ReservationEntity)
            .join(ReservationEntity.users)
    ...
    def staff_checkin_reservation(
        ...
        self._permission_svc.enforce(subject, "equipment.reservation.manage", f"user/*")
    ...
    def _initialize_equipment_availability_dict(
    ...
    def _remove_reservations_from_availability(
        self,
        equipment_availability_dict: dict[int, EquipmentAvailability],
    ...
    def _prune_equipment_below_availability_threshold(
        ...
        available_equipment: list[EquipmentAvailability] = []
        for x in equipment:
            x.filter_time_ranges_below(threshold)
            if len(x.availability) > 0:
                available_equipment.append(x)
        return available_equipment
